/* Experience — vertical Zeitstrahl. Mono year axis, hairline spine with
   accent nodes, role entries. Composes Tag / SectionLabel / Badge. */
const { Tag, SectionLabel, Badge } = window.MauriceDPpenDesignSystem_3628c1;

function Experience() {
  const TIMELINE = window.TIMELINE;
  return (
    <section id="experience" className="section bp-dots">
      <SectionLabel index="03" accent>Experience</SectionLabel>
        ...
        {TIMELINE.map((e, i) => (
          <div key={e.year} className="tl-row" style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 36, position: 'relative' }}>
            ...
            <div style={{ position: 'relative', paddingLeft: 36, paddingBottom: i === TIMELINE.length - 1 ? 0 : 80 }}>
              ...
              <h3 style={{ margin: 0, fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: 27, letterSpacing: '-0.02em', color: 'var(--ink-0)' }}>
                {e.role}
              </h3>
